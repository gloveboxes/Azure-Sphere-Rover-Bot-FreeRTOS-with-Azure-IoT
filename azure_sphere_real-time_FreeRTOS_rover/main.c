...
static void MeasureDistance(void* pParameters)
{
	...
	while (true)
	{
		distance_left = get_distance(HCSR04_LEFT, 10000);
		distance_right = get_distance(HCSR04_RIGHT, 10000);

		if (!isnan(distance_left) && !isnan(distance_right))
		{
...
_Noreturn void RTCoreMain(void)
{
	...
	xTaskCreate(LedTask, "LED Task", APP_STACK_SIZE_BYTES, NULL, 7, NULL);
